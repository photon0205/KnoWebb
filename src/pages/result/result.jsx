...
export default function Result() {
  ...
  useEffect(() => {
    const scores = [
      ...
      localStorage.getItem("puzzle_score"),
      localStorage.getItem("slider_score"),
      localStorage.getItem("jumbleword_score"),
      localStorage.getItem("order_score"),
    ];
    let final = 0;
    for (let i = 0; i < 5; i++) final += parseInt(scores[i]);
    setfinal_score(final);
    var urls = [];
    console.log(scores);
    const func = async () => {
      for (let i = 0; i < 5; i++) {
      ...
      {load ? (
        ...
        <>
          <div className="heading">
            ...
            <span>
              The Difference PSF shows how your image and the ideal image
              differed. The whiter it is, the better you did!
            </span>
            <button className="footer" onClick={reset}>
              Reset
            </button>
          </div>
